...
static const NSUInteger kRadioButtonWidth=45;
...
+(void)buttonSelected:(RadioButton*)radioButton{
    ...
    if (rb_instances)
    {
        for (int i = 0; i < [rb_instances count]; i++)
        {
            ...
            /*
            //NSLog(@"%@",[rb_instances objectAtIndex:i]);
            
            //NSLog(@"radioButton.groupId %@",radioButton.groupId);
            
            //NSLog(@"button.groupId %@",button.groupId);
            
            //NSLog(@"radioButton %@",radioButton);
